...
const SESSION_STORAGE_KEY = 'hashfox_session';
...
function getTargetProgramId(): PublicKey {
	return new PublicKey(HASHFOX_PROGRAM_ID);
}
...
export function deriveHashfoxSessionTokenPda(
	sessionSigner: PublicKey,
	authority: PublicKey
): PublicKey {
...
export class SessionKeyManager {
	...
	private async revokeAndSweepOnChain(
		...
		if (signerBal > 0) tx.partialSign(sessionKp);
	...
	async createSession(
		...
		if (this.createSessionInFlight) throw new Error('Session funding is already in progress');
				...
				if (prior.authority !== authority.toBase58()) {
					...
					if (sk.length !== 64) {
						...
						try {
							...
							throw new Error(
								'Could not close your previous session on-chain. Try again or use End session and retry.'
							);
						}
					}
				}
	...
	async revokeSession(wallet: {
		...
		try {
			await this.revokeAndSweepOnChain(
				wallet,
				sessionKp,
				new PublicKey(session.sessionTokenPDA)
			);
			this.clearSession();
		} catch (e) {
