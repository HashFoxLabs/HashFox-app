...
export function tradingPositionKey(positionId: number): string {
...
export async function syncClosedTradingPositions(
	...
	await Promise.all(
		closed.map(async (p) => {
			const positionKey = tradingPositionKey(p.positionId);
	...
	return fetchPostedStateByPositionKeys(
		walletAddress,
		closed.map((p) => tradingPositionKey(p.positionId))
	);
}
