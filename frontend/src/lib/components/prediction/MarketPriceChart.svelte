<script lang="ts">
	...
	async function initChart() {
		...
		chart.subscribeCrosshairMove((param: any) => {
			...
			const ttWidth = 210;
			tooltipX =
				param.point.x + 18 + ttWidth > cw ? param.point.x - ttWidth - 8 : param.point.x + 18;
	...
	async function createSeries() {
		...
		try {
			...
			error = e?.message ?? 'Chart init failed';
			loading = false;
		}
	}
	...
	async function updateData() {
		...
		try {
			...
			error = e?.message ?? 'Chart data failed';
		} finally {
			loading = false;
		}
	}
...
<div class="mpc-wrap">
	...
	<div class="mpc-canvas-wrap">
		...
		{#if tooltipVisible}
			<div class="mpc-tooltip" style="left:{tooltipX}px; top:{tooltipY}px">
...
<style>
	...
	.mpc-tt-dot { width: 7px; height: 7px; border-radius: 50%; flex-shrink: 0; }
	.mpc-tt-label {
		flex: 1;
		font-size: 11px;
		color: #aaa;
